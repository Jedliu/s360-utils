import { Cell, CellValue, DocRange, Workbook } from '@s360-utils/exceljs';
...
export class Scope {
  ...
  public applyMerge(): void {
    // TODO refactor names
    const tws = this.template.worksheets[this.templateCell.ws] as any;
    ...
    if (tc.isMerged && tc.address === (tc.master && tc.master.address)) {
      ...
      const dr = new DocRange();
      dr.setTLBR(top, left, bottom, right);
      const range = dr.shortRange as any;
