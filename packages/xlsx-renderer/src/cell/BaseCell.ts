...
export /* abstract */ class BaseCell {
  ...
  protected static isMasterCell(scope: Scope) {
    const templateCell = scope.template.worksheets[
      scope.templateCell.ws
    ].getCell(scope.templateCell.r, scope.templateCell.c);
  ...
  constructor() {
    ...
    throw new TypeError(
      `Cannot construct ${BaseCell.name} instances directly. It's abstract.`
    );
  }
