...
export class FinishCell extends BaseCell {
  ...
  protected static getCondition(scope: Scope): boolean {
    ...
    return !!args[2]
      ...
      .reduce(
        (p, c) =>
          typeof p === 'undefined' || typeof p[c] === 'undefined' || p[c],
        scope.vm
      );
  }

  public override apply(scope: Scope): FinishCell {
    super.apply(scope);
    ...
    if (FinishCell.getCondition(scope)) {
      ...
      const wso = scope.output.addWorksheet(
        `Sheet ${scope.outputCell.ws + 1}`,
        wst
      ); // todo if append , problems may happen
