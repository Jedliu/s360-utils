...
export type SplitMapper<TItem> = ValueMapper<TItem[]> & {
  ...
  itemMapper: <TMapper>(
    itemMapper: ValueMapper<TMapper>
  ) => SplitMapper<TMapper>;
};
...
const factory = <TItem>(
  options: Readonly<ISplitMapperOptions<TItem>>
): SplitMapper<TItem> => {
  const mapper: SplitMapper<TItem> = (value: string) =>
    value.split(options.separator).map(options.itemMapper);

  mapper.separator = (separator) => factory({ ...options, separator });
  mapper.itemMapper = <TMapper>(itemMapper: ValueMapper<TMapper>) =>
    factory<TMapper>({ ...options, itemMapper });
