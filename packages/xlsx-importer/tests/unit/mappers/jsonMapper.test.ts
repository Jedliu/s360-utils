...
describe('UNIT TEST: src/mappers/', () => {
  describe('jsonMapper', () => {
    ...
    dataProvider.forEach(({ inValue, expectedResult }) => {
      it(`jsonMapper for input "${inValue}" SHOULD return "${JSON.stringify(
        expectedResult
      )}"`, () => {
        expect(jsonMapper(inValue as string)).toEqual(expectedResult);
      });
    });

    it('Should return default value on error', () => {
      const mapper = jsonMapper.default({ a: 1 });
      expect(mapper('invalid')).toEqual({ a: 1 });
    });
  });
});
